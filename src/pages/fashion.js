...
import latestFashionArticles from './_articles/fashion/latestFashionArticles.js';
import allFashionArticles from './_articles/fashion/allFashionArticles.js';
...
const LatestCards = () => {
  ...
  return latestFashionArticles.map(renderCard);
};
...
const AllCards = () => {
  ...
  return allFashionArticles.map(renderCard);
};

export default function Fashion() {
  return (
    <Layout title="Fashion" description="The fashion hub for Fly Falcons Fly News, a student run news website.">
      <div className="grid lg:grid-cols-2 lg:grid-rows-2 p-4 pt-8 gap-4 grid-cols-1 grid-rows-3">
        <h1 className="lg:pl-6 text-5xl lg:text-left text-center">Fashion</h1>  
        <div className="lg:justify-self-end lg:order-2 order-3 justify-self-center">
          <a href="/articles/fashion" className="relative inline-flex items-center py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 outline outline-2 group">
        ...
        <p className="lg:pl-8 order-2 lg:order-3 lg:text-left text-center">The fashion hub for Fly Falcons Fly News.</p>
      </div>
