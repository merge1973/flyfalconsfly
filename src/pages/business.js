...
const LatestCards = () => {
  ...
  return latestBusinessArticles.map(renderCard);
};
...
const AllCards = () => {
  ...
  return allBusinessArticles.map(renderCard);
};

export default function Business() {
  return (
    <Layout title="Business" description="The business hub for Fly Falcons Fly News, a student run news website.">
      <div className="grid grid-cols-2 grid-rows-1 p-4 pt-8 gap-4">
        <h1 className="pl-6 text-5xl">Business</h1>
        <a href="/articles/business" className="justify-self-end w-1/2 relative inline-flex items-center py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 outline outline-2 group">
          ...
          <span className="relative w-full text-left transition-colors duration-200 ease-in-out group-hover:text-white">View all articles</span>
        </a>
      </div>
      <p className="pl-12">The business hub for Fly Falcons Fly News.</p>
