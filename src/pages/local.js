...
import latestLocalArticles from './_articles/local/latestLocalArticles.js';
import allLocalArticles from './_articles/local/allLocalArticles.js';
...
const LatestCards = () => {
  ...
  return latestLocalArticles.map(renderCard);
};
...
const AllCards = () => {
  ...
  return allLocalArticles.map(renderCard);
};

export default function Local() {
  return (
    <Layout title="Local" description="The local hub for Fly Falcons Fly News, a student run news website.">
      <div className="grid lg:grid-cols-2 lg:grid-rows-2 p-4 pt-8 gap-4 grid-cols-1 grid-rows-3">
        <h1 className="lg:pl-6 text-5xl lg:text-left text-center">Local</h1>  
        <div className="lg:justify-self-end lg:order-2 order-3 justify-self-center">
          <a href="/articles/local" className="relative inline-flex items-center py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 outline outline-2 group">
        ...
        <p className="lg:pl-8 order-2 lg:order-3 lg:text-left text-center">The local hub for Fly Falcons Fly News.</p>
      </div>
